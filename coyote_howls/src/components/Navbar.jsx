...
import {
  ...
  NavLink,
  useNavigate,
} from "react-router-dom"; /* special component from the react router library to change the url when you click the buttons in nav */
...
export const Navbar = () => {
  ...
  const [menuOpen, setMenuOpen] =
    useState(
      false
    ); /* useState to determine if the menu pull down should be opened or closed */
  ...
  return (
    <nav>
      <div className="left_nav">
        ...
        {user.isLoggedIn &&
          (user.role === "student" ? (
            <NavLink to="/StudentDashboard" className="dashboard_link">
              Dashboard
            </NavLink>
          ) : (
            <NavLink to="/FacultyDashboard" className="dashboard_link">
              Dashboard
            </NavLink>
          ))}
      </div>
      <div
        className="menu"
        onClick={() => {
          /* adds the pull down menu when screen is small on top right and the action click for */
